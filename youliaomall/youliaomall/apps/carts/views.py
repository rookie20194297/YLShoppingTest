...
class CartsView(View):
    ...
    def put(self, request):
        ...
        if user.is_authenticated:
            ...
            # 修改勾选状态
            if selected:
                pl.sadd('selected_%s' % user.id, sku_id)#添加勾选状态的商品
            else:
                pl.srem('selected_%s' % user.id, sku_id)#去除没有勾选状态的商品
            # 执行
            pl.execute()
    ...
    def delete(self,request):
        ...
        if user is not None and user.is_authenticated:
            ...
            redis_conn = get_redis_connection('carts')
            pl = redis_conn.pipeline()
            pl.hdel('carts_%s'%user.id,sku_id)#删除
            #注意同时也要删除勾选状态
            pl.serm('selected_%s'%user.id,sku_id)
            pl.execute()
