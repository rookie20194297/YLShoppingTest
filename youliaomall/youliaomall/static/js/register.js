var vm = new Vue({
    ...
    methods: {
        ...
        // 生成一个图片验证码的编号，并设置页面中图片验证码img标签的 src属性
        generate_image_code: function () {
            // 生成一个编号 : 严格一点的使用uuid保证编号唯一， 不是很严谨的情况下，也可以使用时间戳
            this.image_code_id = this.generateUUID();
            // 设置页面中图片验证码img标签的src属性
            this.image_code_url = this.host + "/verification/image_code/" + this.image_code_id + "/";
        ...
        check_username: function () {
            ...
            let url = "/register/username/" + this.username + "count/"
        ...
        check_mobile: function () {
            ...
            let url = "/register/mobile/" + this.mobile + "/"
            axios.get(url).then((response) => {
                if (response.data.count != 0) {
                    this.error_mobile = true;
        ...
        send_sms_code: function () {
            ...
            // 请求短信验证码
            let url = '/verification/sms_codes/' + this.mobile + '/' + this.image_code + '/' + this.image_code_id + '/';
            axios.get(url, {
                responseType: 'json'
            })
                .then(response => {
